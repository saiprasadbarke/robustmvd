...
def eval(args):
    ...
    if args.model is None:
        logging.info(
            f"No model specified. Available models are: {', '.join(list_models())}"
        )
        return

    if args.eval_type is None:
        logging.info(
            f"No evaluation type specified. Available evaluation types are: {', '.join(list_evaluations())}"
        )
        return

    if (
        args.eval_type != "robustmvd" and args.dataset is None
    ):  # or dataset not available
        datasets = list_datasets(dataset_type=args.eval_type, no_dataset_type=True)
        logging.info(
            f"No dataset specified. Available datasets are: {', '.join(datasets)}"
        )
        return
    ...
    if args.eval_type != "robustmvd":
        logging.info()
        logging.info(
            f"Evaluating {args.model} model on dataset {args.dataset} in the {args.eval_type} evaluation setting.\n"
        )
        dataset = create_dataset(
            dataset_name_or_path=args.dataset,
            dataset_type=args.eval_type,
        ...
        logging.info()
        logging.info(
            f"Evaluating {args.model} model on the Robust Multi-view Depth Benchmark.\n"
        )
        dataset = None
    ...
    qualitatives = (
        args.qualitatives if args.qualitatives is not None else args.num_qualitatives
    )
    ...
    eval(
        ...
        eth3d_size=args.eth3d_size,
        kitti_size=args.kitti_size,
        dtu_size=args.dtu_size,
        scannet_size=args.scannet_size,
        tanks_and_temples_size=args.tanks_and_temples_size,
        eval_name=args.eval_name,
        finished_iterations=args.finished_iterations,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--eval_type",
        help=f"Evaluation setting. Options are: {', '.join(list_evaluations())}",
    )
    parser.add_argument(
        "--eval_name", help="Name of the evaluation. Optional.", type=str
    )
    ...
    parser.add_argument(
        "--finished_iterations",
        type=int,
        help="Number of iterations that the model was trained. Only used for logging. Optional.",
    )
    parser.add_argument(
        "--weights",
    ...
    parser.add_argument(
        "--num_samples",
        type=int,
        help="Number of samples to be evaluated. Default: evaluate all.",
    )
    parser.add_argument(
        "--samples",
        type=int,
    ...
    parser.add_argument(
        "--max_source_views",
        type=int,
        help="Maximum number of source views to use for evaluation. "
    ...
    parser.add_argument(
        "--eval_uncertainty",
        action="store_true",
        help="Evaluate predicted depth uncertainty.",
    )

    parser.add_argument(
        "--log_dir",
        help="Path to folder for tensorboard and wandb logs. Optional. Default: use output dir.",
    )
    parser.add_argument(
        "--no_tensorboard",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=2,
        help="Input image size in the format (height, width). "
