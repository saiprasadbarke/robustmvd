...
class MultiViewDepthTraining:
    ...
    def __init__(
        ...
        **_,
        ...
        self.inputs = (
            list(set(inputs + ["images"]))
            if inputs is not None
            else ["images", "intrinsics", "poses"]
        )

        batch_augmentations = [] if batch_augmentations is None else batch_augmentations
        batch_augmentations = (
            batch_augmentations
            if isinstance(batch_augmentations, list)
            else [batch_augmentations]
        )
    ...
    def _init_dirs(self):
        ...
        self.weights_only_checkpoints_dir = osp.join(
            self.out_dir, "weights_only_checkpoints_dir"
        )
    ...
    def __call__(self):
        ...
        while should_continue():
            for iter_in_epoch, sample in enumerate(self.dataloader):
                with writer.TimeWriter(
                    name="00_overview/train_sec_iter",
                    step=self.finished_iterations,
                    write=should_log_loss(),
                    avg_over_steps=True,
                    update_eta=True,
                ):
                    ...
                    loss, sub_losses, pointwise_losses = self.loss(
                ...
                if should_print():
                    ...
                    time_per_iteration = (
                        end_print_interval - start_print_interval
                    ) / steps_since_print
                    print(
                        f"Iteration {self.finished_iterations}/{self.max_iterations} - "
                ...
                if (
                    ...
                    < self.finished_iterations
                    < self.max_iterations
                ):
                    if (
                        time.time() - last_checkpoint_time
                        > 60 * self.save_checkpoint_interval_min
                    ):
                        self._save_all()
                        last_checkpoint_time = time.time()
    ...
    def _setup_savers(self):
        ...
        self.saver_all = TrainStateSaver(
            model=self.model,
        ...
        self.saver_weights_only = WeightsOnlySaver(
            model=self.model,
    ...
    def _get_all_checkpoints(self):
        checkpoints = self.saver_all.get_checkpoints(
            include_iteration=True
        ) + self.saver_weights_only.get_checkpoints(include_iteration=True)
    ...
    def _restore_weights(self):
        ...
        if len(all_checkpoints) > 0:
            ...
            if self.saver_all.has_checkpoint(newest_checkpoint):
                ...
                raise ValueError(
                    f"Checkpoint {newest_checkpoint} does not belong to any of the known savers."
                )
    ...
    def _log_all(
        self, sample_inputs, sample_gt, pred, aux, loss, sub_losses, pointwise_losses
    ):
    ...
    def _log_in_data(self, sample_inputs, sample_gt):
        ...
        writer.put_tensor(
            name=f"{base_name}/00_key_image",
            tensor=image_key,
            step=self.finished_iterations,
            full_batch=self.log_full_batch,
        )
        writer.put_tensor_list(
            name=f"{base_name}/01_source_images",
            tensor=images_source,
            step=self.finished_iterations,
            full_batch=self.log_full_batch,
        )
        ...
        writer.put_tensor(
            name=f"{base_name}/03_gt_invdepth",
            tensor=gt_invdepth,
            step=self.finished_iterations,
            full_batch=self.log_full_batch,
            invalid_values=[0],
    ...
    def _log_pred(self, pred, aux):
        ...
        writer.put_tensor(
            name=f"{base_name}/00_pred_depth",
            tensor=pred_depth,
            step=self.finished_iterations,
            full_batch=self.log_full_batch,
        )
        ...
        if "depth_uncertainty" in pred:
            ...
            writer.put_tensor(
                name=f"{base_name}/01_pred_depth_uncertainty",
                tensor=pred_depth_uncertainty,
                step=self.finished_iterations,
                full_batch=self.log_full_batch,
            )

        writer.put_tensor_dict(
            name=f"{base_name}/02_aux",
            tensor=aux,
            step=self.finished_iterations,
            full_batch=self.log_full_batch,
        )
    ...
    def _log_loss(self, loss, sub_losses, pointwise_losses, scalars_only=True):
        base_name = "03_loss"
        writer.put_scalar(
            name="00_overview/00_loss", scalar=loss, step=self.finished_iterations
        )
        writer.put_scalar(
            name=f"{base_name}/00_total_loss",
            scalar=loss,
            step=self.finished_iterations,
        )

        writer.put_scalar_dict(
            name=f"{base_name}/01_sub", scalar=sub_losses, step=self.finished_iterations
        )

        if not scalars_only:
            writer.put_tensor_dict(
                name=f"{base_name}/02_qual",
                tensor=pointwise_losses,
                step=self.finished_iterations,
            )
    ...
    def _log_optim(self):
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            lr = param_group["lr"]
            writer.put_scalar(
                name=f"00_overview/lr/group_{i}",
                scalar=lr,
                step=self.finished_iterations,
            )
