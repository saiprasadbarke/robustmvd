...
class Visualization:
    def __init__(
        self, col, row, visualization_type, load_fct, name=None, colspan=1, rowspan=1
    ):
        self.col = col
        self.row = row
    ...
    def create_display(self, manager):
        display = create_display_by_visualization_type(
            self.visualization_type, manager, label=self.name
        )
        return display
