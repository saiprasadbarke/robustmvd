...
DTU_TRAIN_SCENES = ["scan2","scan6","scan7","scan8","scan14","scan16","scan18","scan19","scan20",
                    "scan22","scan30","scan31","scan36","scan39","scan41","scan42","scan44","scan45",
                    "scan46","scan47","scan50","scan51","scan52","scan53","scan55","scan57","scan58",
                    "scan60","scan61","scan63","scan64","scan65","scan68","scan69","scan70","scan71",
                    "scan72","scan74","scan76","scan83","scan84","scan85","scan87","scan88","scan89",
                    "scan90","scan91","scan92","scan93","scan94","scan95","scan96","scan97","scan98",
                    "scan99","scan100","scan101","scan102","scan103","scan104","scan105","scan107","scan108",
                    "scan109","scan111","scan112","scan113","scan115","scan116","scan119","scan120","scan121",
                    "scan122","scan123","scan124","scan125","scan126","scan127","scan128"]
...
class DTUScene:
    def __init__(self, root):
        ...
        assert len(set(self.depths).intersection(set(self.intrinsics)).intersection(set(self.min_depths.keys())).intersection(set(self.max_depths.keys())).intersection(set(self.poses))) == len(self.images), f"{len(set(self.depths)).intersection(set(self.intrinsics)).intersection(set(self.min_depths.keys())).intersection(set(self.max_depths.keys())).intersection(set(self.poses))} not equal to {len(self.images)}"
