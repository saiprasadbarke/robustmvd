...
class MultiViewDepthEvaluation:
    ...
    def _evaluate(self):
        for sample_num, (sample_idx, sample) in enumerate(
            ...
            if self.verbose:
                logging.info(
                    f"Processing sample {self.cur_sample_num+1} / {len(self.sample_indices)} "
            ...
            max_source_views = (
                ...
                else len(ordered_source_indices)
            )
            ...
            if self.eval_uncertainty:
                uncertainty_metrics = self._compute_uncertainty_metrics(
                    sample_inputs=cur_sample_inputs,
            ...
            if should_qualitative:
                qualitatives = self._compute_qualitatives(
                    sample_inputs=sample_inputs, sample_gt=sample_gt, pred=best_pred
                )
            ...
            if self.verbose:
                logging.info(
                    f"Sample with index {self.cur_sample_idx} has AbsRel={best_metrics['absrel']} "
    ...
    def _postprocess_sample_and_output(self, sample_inputs, sample_gt, pred):
        ...
        if "depth_uncertainty" in pred:
            ...
            pred_depth_uncertainty = skimage.transform.resize(
                pred_depth_uncertainty, gt_depth.shape, order=0, anti_aliasing=False
            )
    ...
    def _compute_uncertainty_metrics(self, sample_inputs, sample_gt, pred):
        ...
        sparsification_oracle = sparsification(
            gt=gt_depth,
            pred=pred_depth,
            uncertainty=oracle_uncertainty,
            mask=pred_mask,
            show_pbar=self.verbose,
            pbar_desc="            Oracle sparsification",
        )
        sparsification_pred = sparsification(
            gt=gt_depth,
            pred=pred_depth,
            uncertainty=pred_depth_uncertainty,
            mask=pred_mask,
            show_pbar=self.verbose,
    ...
    def _get_layout(self):
        ...
        key_img_visualization = Visualization(
            col=0,
            row=0,
            visualization_type="image",
            load_fct=load_key_img,
            name="Key Image",
        )
        ...
        gt_depth_visualization = Visualization(
            col=0,
            row=1,
            visualization_type="float",
            load_fct=load_gt_depth,
            name="GT Depth",
        )
        ...
        gt_invdepth_visualization = Visualization(
            col=1,
            row=1,
            visualization_type="float",
            load_fct=load_gt_invdepth,
            name="GT Inverse Depth",
        )
        ...
        gt_depth_mask_visualization = Visualization(
            col=2,
            row=1,
            visualization_type="float",
            load_fct=load_gt_depth_mask,
            name="GT Mask",
        )
        ...
        pred_depth_visualization = Visualization(
            col=0,
            row=2,
            visualization_type="float",
            load_fct=load_pred_depth,
        ...
        pred_invdepth_visualization = Visualization(
            col=1,
            row=2,
            visualization_type="float",
            load_fct=load_pred_invdepth,
            name="Predicted Inverse Depth",
        )
        ...
        pointwise_absrel_visualization = Visualization(
            col=2,
            row=2,
            visualization_type="float",
            load_fct=load_pointwise_absrel,
        ...
        if self.eval_uncertainty:
            ...
            pred_depth_uncertainty_visualization = Visualization(
                col=3,
                row=2,
                visualization_type="float",
                load_fct=load_pred_depth_uncertainty,
                name="Predicted Depth Uncertainty",
            )
