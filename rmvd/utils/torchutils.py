...
def transform_from_rot_trans(R, t, dtype=torch.float32):
    ...
    if R.ndim == 2:
        if t.ndim == 1:
            ...
            raise ValueError(
                "When the rotation has no batch dimension, the translation must have 1 or 2 dimensions, but has {} dimensions.".format(
                    t.ndim
                )
            )
        ...
        R = R.reshape(3, 3)
        T = torch.vstack(
            (torch.hstack([R, t]), torch.tensor([0, 0, 0, 1], device=R.device))
        ).to(dtype)
        ...
        if t.ndim == 2:
            ...
            raise ValueError(
                "When the rotation has a batch dimension, the translation must have 2 or 3 dimensions, but has {} dimensions.".format(
                    t.ndim
                )
            )
        ...
        last_row = torch.cat(
            [
                torch.tensor([0, 0, 0, 1], dtype=dtype, device=T.device)
                .unsqueeze(0)
                .unsqueeze(0)
            ]
            * N
        ...
        raise ValueError(
            "Rotation must have 2 or 3 dimensions (after squeezing), but has {} dimensions.".format(
                R.ndim
            )
        )
...
def trans_from_transform(T):
    ...
    if T.ndim == 2:
        ...
        raise ValueError(
            "Transform must have 2 or 3 dimensions (after squeezing), but has {} dimensions.".format(
                T.ndim
            )
        )
...
def rot_from_transform(T):
    ...
    if T.ndim == 2:
        ...
        raise ValueError(
            "Transform must have 2 or 3 dimensions, but has {} dimensions.".format(
                T.ndim
            )
        )
...
def compute_depth_range(
    ...
    assert (
        depth is not None or invdepth is not None
    ), "Either depth or invdepth must be provided."
    ...
    if invdepth is not None:
        ...
        for invdepth_sample, mask_sample in zip(invdepths, masks):
            if mask_sample.any():
                ...
                min_invdepth = torch.tensor(
                    1.0 / default_max_depth,
                    dtype=invdepth_sample.dtype,
                    device=invdepth_sample.device,
                )
                max_invdepth = torch.tensor(
                    1.0 / default_min_depth,
                    dtype=invdepth_sample.dtype,
                    device=invdepth_sample.device,
                )
            min_depth = 1.0 / max_invdepth
            max_depth = 1.0 / min_invdepth
            min_depths.append(min_depth)
            max_depths.append(max_depth)
