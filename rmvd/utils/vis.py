...
_DEFAULT_FONTS = {
    _DEFAULT_FONT_SIZE: ImageFont.truetype(_DEFAULT_FONT_PATH, _DEFAULT_FONT_SIZE)
    ...
    if isinstance(imgs[0], Image.Image):
        ...
        for i, img in enumerate(imgs):
            if img.width != max_width or img.height != max_height:
                imgs[i] = img.resize(
                    size=(max_width, max_height), resample=Image.NEAREST
                )
    else:  # np, shape CHW
        ...
        for i, img in enumerate(imgs):
            if img.shape[2] != max_width or img.shape[1] != max_height:
                imgs[i] = skimage.transform.resize(
                    img,
                    [img.shape[0], max_height, max_width],
                    order=0,
                    preserve_range=True,
                )
    return imgs
...
def vis_2d_array(arr, full_batch=False, batch_labels=None, **kwargs):
    ...
    assert (
        2 <= arr.ndim <= 4
    ), f"2d array must have 2, 3 or 4 dimensions, but got shape {arr.shape}"
    if arr.ndim == 4:
        assert arr.shape[1] == 1, (
            f"First dimension in a 2d array with shape {arr.shape} must "
    ...
    if full_batch:
        ...
        for idx, ele in enumerate(arr):
            if batch_labels is not None:
                assert (
                    "label" not in kwargs
                ), "It is not possible to use batch_labels and label argument at the same time."
...
def _vis_single_2d_array(
    ...
    assert (
        arr.ndim == 2
    ), f"Single 2d array must have 2 dimension, but got shape {arr.shape}"
    arr = make_np(arr)
    ...
    draw_text = _get_draw_text(
        text, label, text_off, image_range_text, image_range_text_off
    )
...
def vis_image(img, full_batch=False, batch_labels=None, **kwargs):
    ...
    assert (
        3 <= img.ndim <= 4
    ), f"Image array must have 3 or 4 dimensions, but got shape {img.shape}"
    if img.ndim == 3:
        assert img.shape[0] == 3, (
            f"First dimension in a image array with shape {img.shape} must "
            f"be 3, but got {img.shape[0]}."
        )
    if img.ndim == 4:
        assert img.shape[1] == 3, (
            f"Second dimension in a image array with shape {img.shape} must "
...
def _vis_single_image(
    ...
    assert (
        img.ndim == 3
    ), f"Single image array must have 3 dimension, but got shape {img.shape}"
    img = make_np(img)
    ...
    (
        ...
        img, clipping, upper_clipping_thresh, lower_clipping_thresh, invalid_values
    )
