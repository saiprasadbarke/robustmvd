    ...
    def get_checkpoints(self, include_iteration=False):
        if self.full_path is not None:
            ...
            checkpoints = get_checkpoints(
                path=self.base_path, base_name=self.base_name, include_iteration=False
            )

        if include_iteration:
            checkpoints = [
                (iteration_from_path(checkpoint), checkpoint)
...
class TrainStateSaver:
    ...
    def load(self, model=None, optim=None, scheduler=None, full_path=None):
        ...
        out_dict = load_all(
            path=checkpoint, model=model, optim=optim, scheduler=scheduler
        )
        return out_dict
...
def get_checkpoints(path, base_name=None, include_iteration=False):
    ...
    if include_iteration:
        checkpoints = [
            (iteration_from_path(checkpoint), checkpoint) for checkpoint in checkpoints
        ]
...
def save_model(
    ...
    if full_path is not None:
        ...
        name = (
            name + "-iter-{:09d}".format(iteration) if iteration is not None else name
        )
        name += ".pt"
