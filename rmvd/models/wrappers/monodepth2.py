...
class Monodepth2_Wrapped(nn.Module):
    def __init__(self, model_name, trained_on_stereo):
        super().__init__()
        # Set path of the files to add the model
        import sys
        ...
        # Verify if the weight files exist otherwise print error
        ...
        print(
            f"Using model weights from {encoder_weights_path} and {decoder_weights_path}."
        )
        encoder_weights = torch.load(encoder_weights_path)
        filtered_encoder_weights = {
            k: v for k, v in encoder_weights.items() if k in self.encoder.state_dict()
        }
...
def monodepth2_mono_stereo_1024x320_wrapped(
...
@register_model(trainable=False)
def monodepth2_mono_stereo_640x192_wrapped(
    pretrained=True, weights=None, train=False, num_gpus=1, **kwargs
):
    assert pretrained and (
        weights is None
